...
export default function PromptsPage() {
  ...
  const getPromptDescription = (id: number) => {
    switch (id) {
      case SYSTEM_PROMPT_IDS.TIMETABLE_EXTRACTION:
        return "Extracts timetable data from PDF and image uploads into the shared structured timetable pipeline";
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <SiteHeader title="System Prompts" subtitle="LLM Prompts" />
      <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        <div className="flex items-center justify-between space-y-2">
          <div>
            <h2 className="text-3xl font-bold tracking-tight">System Prompts</h2>
            <p className="text-muted-foreground">
              Manage and edit AI system prompts for the BrainMo services
            </p>
          </div>
        </div>
